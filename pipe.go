...
// See https://github.com/diaz-jared/rueidis/pull/691
	...
	for err == nil {
		if ones[0], multi, ch = p.queue.NextWriteCmd(); ch == nil {
			...
			if flushDelay != 0 && p.loadWaits() > 1 { // do not delay for sequential usage
				...
				// https://github.com/diaz-jared/rueidis/issues/379
				var blocked bool
				for i := 0; i < len(multi) && !blocked; i++ {
					blocked = multi[i].IsBlock()
				}
				if !blocked {
					time.Sleep(flushDelay - time.Since(flushStart)) // ref: https://github.com/diaz-jared/rueidis/issues/156
				}
			}
		}
		if ch != nil && multi == nil {
		...
		for _, cmd := range multi {
			...
			if cmd.IsUnsub() { // See https://github.com/diaz-jared/rueidis/pull/691
